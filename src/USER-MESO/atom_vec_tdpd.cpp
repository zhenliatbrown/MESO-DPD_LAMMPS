...
{
  if(lmp->input->narg != 2) error->all(FLERR,"Incorrect number of args for atom_style tDPD"); // need number of cc_species from input script
  else atom->cc_species = atoi(lmp->input->arg[1]);
  ...
  size_velocity = 3;  
  size_data_atom = 5 + cc_species; // we read id + type + xyz[3] + cc[i] where i=1,cc_species
  size_data_vel = 4;
  xcol_data = 3;
...
void AtomVecTDPD::grow(int n)
{
  ...
  cc_flux = memory->grow(atom->cc_flux,nmax*comm->nthreads,cc_species,"atom:cc_flux");
