...
void NTopoBondPartial::build()
{
    ...
    for (m = 0; m < num_bond[i]; m++) {
      ...
      if (atom1 == -1) {
        ...
        if (lostbond == Thermo::ERROR)
          error->one(FLERR, "Bond atoms {} {} missing on proc {} at step {}" + utils::errorurl(30), tag[i],
  ...
  if (all && (me == 0)) error->warning(FLERR, "Bond atoms missing at step {}" + utils::errorurl(30), update->ntimestep);
}
